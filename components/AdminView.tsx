...
const AdminView: React.FC<AdminViewProps> = ({ data, onLogout }) => {
  const { isLoading, participants, judges, categoriesByLevel, performanceLevels, handlers } = data;
  const [activeView, setActiveView] = useState<AppView>('participants');
  ...
  const handleEndScoring = () => {
   ...
   // Need to update participant in scoringState when it changes in the main state
  useEffect(() => {
    if (scoringState) {
      ...
      case 'score-management':
        return (
          <ScoreManager 
            ...
            performanceLevels={performanceLevels}
            onAddPerformanceLevel={handleAddPerformanceLevel}
            onUpdatePerformanceLevel={handleUpdatePerformanceLevel}
            onDeletePerformanceLevel={handleDeletePerformanceLevel}
            levels={LEVELS}
          />
        );
      case 'ranking':
  ...
  const adminViews: AppView[] = ['participants', 'judges', 'scoring', 'score-management', 'ranking'];
    ...
    <div className="min-h-screen text-gray-800 dark:text-gray-200">
      ...
      <header className="bg-white dark:bg-gray-800 shadow-md sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Admin Panel - PBB</h1>
          <Navbar 
            activeView={activeView} 
            setActiveView={setActiveView} 
            views={adminViews}
